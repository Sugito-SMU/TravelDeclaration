...
<script runat="server">
    ...
    protected void lbtnSubmit1_OnClick(object sender, EventArgs e)
    {
        if (ValidateDates())
        {
            string query = @"
select [UserAccountID], [LogDate], [Temperature], [PersContactNum], [Comment] ,case when IsRespiratory = 1 then 'Yes'
     when IsRespiratory = 0 then 'No'
	 Else ''
End as IsRespiratory,
case when IsQuarantine = 1 then 'Yes'
     when IsQuarantine = 0 then 'No'
	 Else ''
End as IsQuarantine
into #tempLog
from V_Trace_Temperature_Report (nolock)
where logDate >= @DateFrom and logDate <= @DateTo;

create index IX1 on #tempLog([UserAccountID], [LogDate]);

select [UserAccountID], [UserName], [OrganisationDesc], [NtLoginAccount], [UserType], [NRIC_FIN_PP], [Email_Smu]
into #tempUserAcct
...
select b.[UserName] as [Name], b.[UserType] as [Student/Staff], b.[NRIC_FIN_PP] as [NRIC/FIN/Passport No.]
, b.[OrganisationDesc] as [School/Office], a.[PersContactNum] as [Contact No.], lower(b.[Email_Smu]) as [SMU Email]
, lower(replace(replace(b.[NtLoginAccount], 'smustu\', ''), 'smustf\', '')) as [SMU Network ID]
, a.[Temperature] as [Temperature Taken], a.[Comment] as [Remarks], a.[logDate] as [Temperature Logging Date & Time]
, replace(convert(varchar, a.[logDate], 106), ' ', '-') as [Temperature Logging Date]
, convert(varchar, a.[logDate], 108) as [Temperature Logging Time],a.IsRespiratory as [Has Respiratory Symptom?], a.IsQuarantine as [Has Been Quarantined?]
from #tempLog a
inner join #tempUserAcct b
	on b.[UserAccountID] = a.[UserAccountID]
order by a.[logDate] desc, b.[UserName] asc;
...
";
            ...
            GenerateDownloadFile(ds, "Temperature_Logging_Report_" + DateTime.Now.ToString("yyyyMMdd_HHmm") + ".xls");
        }
    }
    ...
    protected void lbtnSubmit2_OnClick(object sender, EventArgs e)
    {
        ...
        {
            string query = @"
select [UserAccountID], [LogDate], [Temperature], [PersContactNum], [Comment],case when IsRespiratory = 1 then 'Yes' when IsRespiratory = 0 then 'No' Else '' End as IsRespiratory,
case when IsQuarantine = 1 then 'Yes'
     when IsQuarantine = 0 then 'No'
	 Else ''
End as IsQuarantine
...
select [UserAccountID], [UserName], [OrganisationDesc], [NtLoginAccount], [UserType], [NRIC_FIN_PP], [Email_Smu], [Contact_Mobile]
into #tempUserAcct
from [V_Trace_UserAccount] (nolock)
where isactive = 'Y';
create index IX1 on #tempUserAcct([USERACCOUNTID]);

select a.[UserName] as [Name], a.[UserType] as [Student/Staff], a.[NRIC_FIN_PP] as [NRIC/FIN/Passport No.]
, a.[OrganisationDesc] as [School/Office], a.[Contact_Mobile] as [Contact No.], lower(a.[Email_Smu]) as [SMU Email]
, lower(replace(replace(a.[NtLoginAccount], 'smustu\', ''), 'smustf\', '')) as [SMU Network ID], b.IsRespiratory as [Has Respiratory Symptom?], b.IsQuarantine as [Has Been Quarantined?]
from #tempUserAcct a
left join #tempLog b
	on b.[UserAccountID] = a.[UserAccountID]
...
";
            ...
            System.Data.DataSet ds = GetDataSet(query, parameters);
            GenerateDownloadFile(ds, "Temperature_Not_Logged_Report_" + DateTime.Now.ToString("yyyyMMdd_HHmm") + ".xls");
        }
    }
            ...
            <div class="container-fluid">
                ...
                <h5>
                    <ol style="list-style-type: decimal;">
                        <li>Temperature Logging Report&nbsp;&nbsp;&nbsp;
                            <asp:LinkButton ID="lbtnSubmit1" runat="server" Text="Download" OnClick="lbtnSubmit1_OnClick" />
                        </li>
                        <br />
                        <li>Temperature Logging Report (on who did not log temperature)&nbsp;&nbsp;&nbsp;
